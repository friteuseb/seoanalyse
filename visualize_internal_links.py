...
def visualize_internal_links(crawl_id):
    ...
    for node in G.nodes():
        ...
        short_url = node.split('/')[-1]
        node_trace['text'] += tuple([short_url])
    ...
    fig = go.Figure(data=edge_trace + [node_trace],
                    layout=go.Layout(
                        ...
                        annotations=[dict(
                            text="Visualisation du maillage interne",
                            showarrow=False,
    ...
    # Fonction pour ouvrir l'URL complète au clic
    def open_url(trace, points, state):
        ind = points.point_inds[0]
        url = node_trace['text'][ind]
        webbrowser.open(url)

    node_trace.on_click(open_url)
    
    pio.show(fig)

if __name__ == "__main__":
    crawl_id = input("Entrez l'ID du crawl à visualiser: ")
    visualize_internal_links(crawl_id)
